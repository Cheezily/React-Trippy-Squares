...
var Output = React.createClass({
  render: function() {
    var starMap = this.props.starMap;
    //console.log(starMap);
    return (
      <div>
        {starMap.map(function(thisStar, key) {
          return (<div key={key + 1} className='star' style={thisStar.style}></div>)
        })
        }
      </div>
    );
...
function showStars() {
  ...
  for (var i = 0; i < stars.length; i++) {
    ...
    starStyle = {top: stars[i].yPos + "px",
                ...
                boxShadow: Math.floor(stars[i].shadow) +"px "+ Math.floor(stars[i].shadow) +"px " + Math.floor(stars[i].shadow) + "px #000"
    };

    moveStar(i, starStyle);
    deleteCheck(stars[i], i);
  }

  //target.innerHTML = starDivs;
  ReactDOM.render(<Output starMap={stars} />, document.getElementById('target'));
